...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.dense_embed_conv = nn.Conv2d(384, 256, kernel_size=1) #add by Ray
    ...
    def predict_masks(
        ...
        # dense_prompt_embeddings_resized = F.interpolate(dense_prompt_embeddings, size=image_embeddings.shape[-2:], mode='bilinear', align_corners=False)
        # dense_prompt_embeddings_aligned = self.dense_embed_conv(dense_prompt_embeddings_resized)

        # Resize dense_prompt_embeddings to match the shape of src
        ...
        if dense_prompt_embeddings_resized.shape[1] != image_embeddings.shape[1]:
            dense_prompt_embeddings_resized = nn.Conv2d(dense_prompt_embeddings_resized.shape[1], image_embeddings.shape[1], kernel_size=1)(dense_prompt_embeddings_resized)

        
        src = image_embeddings + dense_prompt_embeddings_aligned
